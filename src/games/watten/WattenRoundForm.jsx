...
export default function WattenRoundForm({ form, onFormChange, team1_players, team2_players, isGespannt, team1_score, team2_score, targetScore, onSubmit, onCancel }) {
  ...
  return (
    <div style={styles.gameForm}>
          ...
          {[2, 3, 4, 5].map(points => (
            <button
              ...
              style={{
                ...
                opacity: isTeamGespannt || (isSpanntPlayed && points !== 2) ? 0.5 : 1,
                cursor: isTeamGespannt || (isSpanntPlayed && points !== 2) ? "not-allowed" : "pointer",
              }}
              onClick={() => handlePointsChange(points)}
              disabled={isTeamGespannt || isSpanntPlayed}
            >
              {points}
            </button>
          ))}
        ...
        <div style={{ marginTop: 16, marginBottom: 8 }}>
          <label style={{ ...styles.label, display: "flex", alignItems: "center", gap: 8 }}>
            <input
              ...
              disabled={isSpanntPlayed || isTeamGespannt}
            />
        ...
        {isGespannt && !isSpanntPlayed && (
          <div style={{ marginBottom: 16 }}>
            <button
              ...
              disabled={isMachine}
            >
              {isSpanntPlayed ? "✓ Gespannt geht (2 Punkte)" : "Gespannt → Geht (gegangen → 2 Punkte)"}
            </button>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 16 }}>
          <div style={{ flex: 1 }}>
            <label style={{ fontSize: 12, color: "#2c1810", marginBottom: 4, display: "block" }}>{team1_players.join(" + ")}</label>
            <input
              list="tricks-team1"
              type="number"
              min="0"
          ...
          <div style={{ flex: 1 }}>
            <label style={{ fontSize: 12, color: "#2c1810", marginBottom: 4, display: "block" }}>{team2_players.join(" + ")}</label>
            <input
              list="tricks-team2"
              type="number"
              min="0"
