...
// Dynamische Imports für Wizard-Komponenten
const WizardScoreSheet = ({ session, registeredPlayers, onBack, onSessionUpdated }) => {
  const Component = React.lazy(() => import("../games/wizard/ScoreSheet.jsx"));
  return (
    <React.Suspense fallback={<div style={{ padding: 20, textAlign: "center" }}>Laden...</div>}>
      <Component session={session} registeredPlayers={registeredPlayers} onBack={onBack} onSessionUpdated={onSessionUpdated} />
    </React.Suspense>
  );
};
...
export default function SessionView({ session, registeredPlayers = [], onBack, onSessionUpdated }) {
  ...
  const handleUndo = async () => {
    ...
    if (res.ok) {
      ...
      if (lastActive) {
        onSessionUpdated({ ...session, history: history.filter((g) => g.id !== lastActive.id) });
      }
    }
  };
  ...
  return (
    <div style={styles.container}>
      ...
      <Scoreboard players={players} balances={balances} history={activeHistory} registeredPlayers={registeredPlayers} />
